## How to Design Software Good
[How to Design Software Good](https://www.haiku-os.org/docs/HIG/index.xml)

## Programming Paradigms for Dummies: What Every Programmer Should Know
[Programming Paradigms for Dummies: What Every Programmer Should Know](https://www.info.ucl.ac.be/~pvr/VanRoyChapter.pdf)

## How Lisp Became God's Own Programming Language
[How Lisp Became God's Own Programming Language](https://twobithistory.org/2018/10/14/lisp.html)

## why you can have a million go routines but only 1000 java threads
[why you can have a million go routines but only 1000 java threads](https://rcoh.me/posts/why-you-can-have-a-million-go-routines-but-only-1000-java-threads/)

## go hits the concurrency nail right on the head
[go hits the concurrency nail right on the head](https://eli.thegreenplace.net/2018/go-hits-the-concurrency-nail-right-on-the-head/)

## holding a program in one's head
[holding a program in one's head](http://paulgraham.com/head.html)


## 100+ Data Structure and Algorithm Interview Questions Answers for Programmersexit
[100+ Data Structure and Algorithm Interview Questions Answers for Programmers](http://www.java67.com/2018/06/data-structure-and-algorithm-interview-questions-programmers.html)

## fred brooks vs dijkstra
[fred brooks vs dijkstra](https://hmijailblog.blogspot.com/2015/08/fred-brooks-vs-dijkstra.html)

## goodbye python hello go
[goodbye python hello go](https://thinkfaster.co/2018/07/goodbye-python-hello-go/)

## if monads are the solution, what is the problem?
[if monads are the solution, what is the problem?](http://danghica.blogspot.com/2018/07/haskell-if-monads-are-solution-what-is.html)

## why google stores billions of lines of code in a single repository
[why google stores billions of lines of code in a single repository](https://cacm.acm.org/magazines/2016/7/204032-why-google-stores-billions-of-lines-of-code-in-a-single-repository/fulltext)

## rust vs go
[rust vs go](http://julio.meroh.net/2018/07/rust-vs-go.html)

## list of all steve yegge articles
[list of all steve yegge articles](http://ratfactor.com/yeggedex.html)

## bitcode demystified
[bitcode demystified](https://lowlevelbits.org/bitcode-demystified/)

## functional data structures
[functional data structures](https://cs.uwaterloo.ca/~plragde/flaneries/FDS/)

## why not haskell
[why not haskell](https://pchiusano.github.io/2017-01-20/why-not-haskell.html)

# what if javascript wins
[what if javascript wins](https://medium.com/@anildash/what-if-javascript-wins-84898e5341a)

## 11 best programming fonts 
[11 best programming fonts](https://itnext.io/11-best-programming-fonts-724283a9ed57)

## the best books on computer science and programming
[the best books on computer science and programming](https://fivebooks.com/best-books/programming-computer-science-ana-bell)

## anatomy of a program in memory
[anatomy of a program in memory](https://manybutfinite.com/post/anatomy-of-a-program-in-memory/)

## the coming software apocalypse
[the coming software apocalypse](https://www.theatlantic.com/technology/archive/2017/09/saving-the-world-from-code/540393/)

## beginers guide to big o notation
[beginers guide to big o notation](http://www.techtalko.com/2018/04/11/beginner-guide-to-big-o-notation-time-and-space-complexity)

## taxonomy of technical debt
[taxonomy of technical debt](https://engineering.riotgames.com/news/taxonomy-tech-debt)

## the basics of protocol buffers
[the basics of protocol buffers](https://www.youtube.com/watch?v=_jQ3i_fyqGA)

## types and why you should care
[types and why you should care](https://www.youtube.com/watch?v=0arFPIQatCU)

## stack overflow survey 2018
[stack overflow survey 2018](https://insights.stackoverflow.com/survey/2018/)

## move fast and fix things
[move fast and fix things](https://githubengineering.com/move-fast/)

## Message Passing and the Actor Model
[Message Passing and the Actor Model](http://dist-prog-book.com/chapter/3/message-passing.html)

## why i hate all articles about design patterns
[why i hate all articles about design patterns](http://www.smashcompany.com/technology/why-i-hate-all-articles-about-design-patterns)

## Go 2017 survey results
[Go 2017 survey results](https://blog.golang.org/survey2017-results)

## the paradigms of programming
[the paradigms of programming](https://blog.acolyer.org/2018/01/29/the-paradigms-of-programming)

## programming notes (free downloads)
[programming notes (free downloads)](http://books.goalkicker.com/)

## what does code readability mean
[what does code readability mean](http://typicalprogrammer.com/what-does-code-readability-mean)

## what orms have tought me - just learn SQL
[what orms have tought me - just learn SQL](http://woz.posthaven.com/what-orms-have-taught-me-just-learn-sql)

## how to be an insanely good developer
[how to be an insanely good developer](https://hackernoon.com/how-to-be-an-insanely-good-developer-936cda639334)

## functional programming jargon
[functional programming jargon](https://functional.works-hub.com/blog/Functional-Programming-Jargon)

## theyre talking behind my back remote workers feel unsupported 1509541381
[theyre talking behind my back remote workers feel unsupported](https://www.wsj.com/articles/theyre-talking-behind-my-back-remote-workers-feel-unsupported-1509541381)

## What did Alan Kay mean by Lisp is the greatest single programming language ever designed
[What did Alan Kay mean by Lisp is the greatest single programming language ever designed](https://www.quora.com/What-did-Alan-Kay-mean-by-Lisp-is-the-greatest-single-programming-language-ever-designed/answer/Alan-Kay-11)

## the coming software apocalypse
[the coming software apocalypse](https://www.theatlantic.com/technology/archive/2017/09/saving-the-world-from-code/540393)

## its time to kill the web
[its time to kill the web](https://blog.plan99.net/its-time-to-kill-the-web-974a9fe80c89)

## 10 common software architectural patterns in a nutshell
[10 common software architectural patterns in a nutshell](https://medium.com/towards-data-science/10-common-software-architectural-patterns-in-a-nutshell-a0b47a1e9013)

## what every developer should know about memory
[what every developer should know about memory](https://lwn.net/Articles/250967)

## technical-interview-performance-by-editor-os-language
[technical-interview-performance-by-editor-os-language](https://triplebyte.com/blog/technical-interview-performance-by-editor-os-language)
[hacker news discussion](https://news.ycombinator.com/item?id=15285927)

## strongly typed languages reduce bugs by 15%
[strongly typed languages reduce bugs by 15%](https://blog.acolyer.org/2017/09/19/to-type-or-not-to-type-quantifying-detectable-bugs-in-javascript)
[reddit discussion](https://www.reddit.com/r/programming/comments/710yaq/strongly_typed_languages_reduce_bugs_by_15)

## monoids what they are why they are useful and what they teach us about software
[monoids what they are why they are useful and what they teach us about software](https://deque.blog/2017/09/13/monoids-what-they-are-why-they-are-useful-and-what-they-teach-us-about-software)

## drowining in technical debt
[drowining in technical debt](https://hackernoon.com/were-drowning-in-tech-debt-why-isn-t-anyone-listening-f4269cb5cc40)

## what do people mean when the say transpiler
[what do people mean when the say transpiler](http://composition.al/blog/2017/07/30/what-do-people-mean-when-they-say-transpiler/)

## what makes a good repl
[what makes a good repl](http://vvvvalvalval.github.io/posts/what-makes-a-good-repl.html)

## kill microservices before it's too late
[kill microservices before it's too late](https://www.youtube.com/watch?v=-UKEPd2ipEk)

## small functions considered harmful
[small functions considered harmful](https://medium.com/@cindysridharan/small-functions-considered-harmful-91035d316c29)

## software engineering != computer science
[software engineering != computer science](http://www.drdobbs.com/architecture-and-design/software-engineering-computer-science/217701907)

## rest and vest millionaire engineers who barely work silicon valley
[rest and vest millionaire engineers who barely work silicon valley](http://www.businessinsider.com/rest-and-vest-millionaire-engineers-who-barely-work-silicon-valley-2017-7)

## epic steve yegge rant
[epic steve yegge rant]https://plus.google.com/110981030061712822816/posts/AaygmbzVeRq

## my first big bill gates review
[my first big bill gates review](https://www.joelonsoftware.com/2006/06/16/my-first-billg-review)

## nuts and bolts of multithreaded programming
[nuts and bolts of multithreaded programming](https://software.intel.com/en-us/articles/nuts-and-bolts-of-multithreaded-programming)

## hashtables 
[hashtables](http://www.zeroequalsfalse.press/2017/02/20/hashtables)

## can your CTO still code
[can your CTO still code](https://www.recode.net/2017/6/15/15332486/cto-code-coding-skills-developer-education-engineer)

## tabs vs spaces - salary
[tabs vs spaces - salary](http://gizmodo.com/the-nerdiest-debate-ever-may-finally-have-a-winner-1796144565)

## keyboard vs mouse
[keyboard vs mouse](https://danluu.com/keyboard-v-mouse/)

## uuid or guid as primary keys - be careful
[uuid or guid as primary keys - be careful](https://tomharrisonjr.com/uuid-or-guid-as-primary-keys-be-careful-7b2aa3dcb439?gi=ef70da1a31e4)

## a brief history of the uuid
[a brief history of the uuid](https://segment.com/blog/a-brief-history-of-the-uuid)

## best website a programmer should visit
[best website a programmer should visit](https://github.com/sdmg15/Best-websites-a-programmer-should-visit)

## hype driven development
[hype driven development](https://blog.daftcode.pl/hype-driven-development-3469fc2e9b22)

## programmer competency matrix
[programmer competency matrix](http://sijinjoseph.com/programmer-competency-matrix/)

## functional design patterns
[functional design patterns](https://www.youtube.com/watch?v=E8I19uA-wGY)

## stack overflow insights survey 2017
[stack overflow insights 2017](https://stackoverflow.com/insights/survey/2017)

## best programming books
[best programming books](http://booksreddit.com/product-category/best-programming-books)

## 
[](https://www.youtube.com/playlist?list=PLzV58Zm8FuBL6OAv1Yu6AwXZrnsFbbR0S)

## free programming books
[free programming books](https://github.com/vhf/free-programming-books)

## what CS Majors should know
[what cs majors should know](http://matt.might.net/articles/what-cs-majors-should-know)

## The Ultimate Reading List for Developers
[The Ultimate Reading List for Developers](https://medium.com/@YogevSitton/the-ultimate-reading-list-for-developers-e96c832d9687#.396mzouuh)
[The Ultimate Reading List for Developers](https://docs.google.com/spreadsheets/d/1jz2dyyDzgZS1zqe_vhY8SYQrhscNzpNfdAkGusWoMUU/edit#gid=0)

## concept, techniques and models
[concepts, techniques and models](https://mitpress.mit.edu/books/concepts-techniques-and-models-computer-programming)
